...
# [Previous code remains the same until the AdvancedHMMTrader class]
...
class AdvancedHMMTrader:
    ...
    class Visualizer:
        """Nested class for generating visualizations"""
        def __init__(self, results, df, signals):
        ...
        def plot_trading_signals(self):
            """Plot price action with trading signals"""
            ...
            # Add buy signals
            buy_signals = self.signals[self.signals['position'] > 0].index
            fig.add_trace(
                go.Scatter(
                    x=buy_signals,
                    y=self.df.loc[buy_signals, 'low'] * 0.99,
                    mode='markers',
            ...
            # Add sell signals
            sell_signals = self.signals[self.signals['position'] < 0].index
            fig.add_trace(
                go.Scatter(
                    x=sell_signals,
                    y=self.df.loc[sell_signals, 'high'] * 1.01,
                    mode='markers',
        ...
        def plot_regime_analysis(self, regime_data):
            """Plot market regime analysis"""
            fig = make_subplots(rows=2, cols=2,
                              subplot_titles=('Volatility Regime',
                                            'Trend Regime',
        ...
        def plot_performance_metrics(self):
            ...
            # Cumulative returns
            returns = self.portfolio['equity'].pct_change()
            ...
            # Drawdown
            drawdown = self.portfolio['equity'] / self.portfolio['equity'].cummax() - 1
            fig.add_trace(
                go.Scatter(
                    x=drawdown.index,
                    y=drawdown,
            ...
            fig.add_trace(
                go.Heatmap(
                    z=monthly_returns.values.reshape(12, -1),
                    x=['Year ' + str(y) for y in range(monthly_returns.index.year.min(),
                                                     monthly_returns.index.year.max() + 1)],
        ...
        def plot_risk_metrics(self):
            ...
            # Risk per trade
            trade_returns = self.portfolio[self.portfolio['shares'] != 0]['returns']
            fig.add_trace(
                go.Histogram(
                    x=trade_returns,
                    name='Trade Returns'
            ...
            risk_adj_returns = rolling_returns.mean() / rolling_returns.std()
            fig.add_trace(
                go.Scatter(
                    x=risk_adj_returns.index,
                    y=risk_adj_returns,
        ...
        def generate_report(self, output_path='trading_report.html'):
            ...
            <body>
                ...
                <ul>
                    <li>Total Return: {self.results['total_return']:.2f}%</li>
                    <li>Sharpe Ratio: {self.results['sharpe_ratio']:.2f}</li>
                    <li>Max Drawdown: {self.results['max_drawdown']:.2f}%</li>
                    <li>Win Rate: {self.results['win_rate']*100:.2f}%</li>
                </ul>
                
                <div id="trading_signals"></div>
                <div id="regime_analysis"></div>
                <div id="performance_metrics"></div>
                <div id="risk_metrics"></div>
                
                <script>
                    {self.plot_trading_signals().to_html(full_html=False, include_plotlyjs=False)}
                    {self.plot_regime_analysis(self.df).to_html(full_html=False, include_plotlyjs=False)}
                    {self.plot_performance_metrics().to_html(full_html=False, include_plotlyjs=False)}
                    {self.plot_risk_metrics().to_html(full_html=False, include_plotlyjs=False)}
                </script>
            </body>
            </html>
            """
    ...
    def visualize_results(self, results, df, signals):
        """Create visualization instance and generate report"""
        visualizer = self.Visualizer(results, df, signals)
        visualizer.generate_report()
        return visualizer

# Example usage with visualization
def run_advanced_strategy_with_viz(data, symbol):
    ...
    signals = trader.generate_enhanced_signals(data, symbol)
    visualizer = trader.visualize_results(results, data, signals)
